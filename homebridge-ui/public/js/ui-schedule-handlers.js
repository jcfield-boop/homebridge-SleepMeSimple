...
if (typeof window.renderScheduleList === 'function') {
     ...
     * Enhanced to mark templates for easier identification and editing
     */
    window.applyScheduleTemplates = function() {
      ...
      // Track the current template being applied (for identification)
      window.currentTemplateOrigin = "";
      ...
      if (weekdayKey && templates[weekdayKey]) {
          ...
          // Set current template origin for identification
          window.currentTemplateOrigin = templates[weekdayKey].name;
          ...
          templates[weekdayKey].schedules.forEach(templateSchedule => {
              ...
              const schedule = {
                  ...
                  // Add template information for improved management
                  isFromTemplate: true,
                  templateSource: templates[weekdayKey].name,
      ...
      if (weekendKey && templates[weekendKey]) {
          ...
          // Set current template origin for identification
          window.currentTemplateOrigin = templates[weekendKey].name;
          ...
          templates[weekendKey].schedules.forEach(templateSchedule => {
              ...
              const schedule = {
                  ...
                  // Add template information for improved management
                  isFromTemplate: true,
                  templateSource: templates[weekendKey].name,
  ...
  /**
   * Export template configuration to JSON format
   * Allows users to see the exact template definition for config.json editing
   * @returns {string} JSON string of template configuration
   */
  window.exportTemplateConfig = function() {
      const templates = window.templates || {};
      
      if (!templates || Object.keys(templates).length === 0) {
          console.error('No templates available to export');
          return '{}';
      }
      
      try {
          // Create a clean export of template configurations
          ...
          // Format JSON with nice indentation for readability
          return JSON.stringify(templateExport, null, 2);
      } catch (error) {
  ...
  /**
   * Preview a template's schedules without applying them
   * @param {string} templateKey - Key of template to preview
   * @returns {Array} Array of schedules from the template
   */
  window.previewTemplate = function(templateKey) {
      ...
      try {
          // Create a preview of the template schedules
          return template.schedules.map(schedule => {
              // Convert temperature if needed
              let temperature = schedule.temperature;
              if (currentUnit === 'F' && typeof window.convertCtoF === 'function') {
                  temperature = Math.round(window.convertCtoF(temperature) * 10) / 10;
              }
   ...
   * Enhanced to show template information
   */
      ...
      try {
          ...
          Object.keys(groupedSchedules).forEach(type => {
            ...
            sortedSchedules.forEach(schedule => {
                ...
                // Add template class if this schedule is from a template
                if (schedule.isFromTemplate) {
                ...
                // Create template badge if from a template
                if (schedule.isFromTemplate && schedule.templateSource) {
                    const templateBadge = document.createElement('span');
                    templateBadge.className = 'template-badge';
                    templateBadge.title = `From template: ${schedule.templateSource}`;
