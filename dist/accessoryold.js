...
export class SleepMeAccessory {
    ...
    setupStatusPolling(initialDelay = 0) {
        ...
        if (initialDelay > 0) {
            this.platform.log.info(`Scheduling initial status check for device ${this.deviceId} in ${initialDelay / 1000} seconds`);
            setTimeout(() => {
        ...
        this.statusUpdateTimer = setInterval(() => {
            ...
            if (timeSinceUserAction < quietPeriod) {
                this.platform.log.debug(`Skipping update, within quiet period after user action (${Math.round(timeSinceUserAction / 1000)}s of ${Math.round(quietPeriod / 1000)}s)`);
                return;
            }
            ...
            if (shouldPoll) {
                this.refreshDeviceStatus().catch(error => {
                    ...
                    this.platform.log.error(`Error updating device status (attempt ${this.failedUpdateAttempts}): ${error instanceof Error ? error.message : String(error)}`);
                });
            }
        }, pollingIntervalMs);
    }
    ...
    async refreshDeviceStatus(isInitialSetup = false) {
        ...
        if (!isInitialSetup) {
            ...
            if (timeSinceUserAction < USER_ACTION_QUIET_PERIOD_MS) {
                this.platform.log.debug(`Skipping scheduled status update, recent user interaction ${Math.round(timeSinceUserAction / 1000)}s ago`);
...
//# sourceMappingURL=accessoryold.js.map