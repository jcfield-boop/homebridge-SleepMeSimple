...
import { MIN_TEMPERATURE_C, MAX_TEMPERATURE_C, COMMAND_DEBOUNCE_DELAY_MS, USER_ACTION_QUIET_PERIOD_MS, InterfaceMode, DEFAULT_INTERFACE_MODE, POLLING_INTERVALS, POLLING_CONTEXTS } from './settings.js';
/**
 ...
 */
function validateTemperature(temperature, fallback = MIN_TEMPERATURE_C) {
...
export class SleepMeAccessory {
    ...
    currentTemperature = MIN_TEMPERATURE_C; // 13°C - meets both current (8°C) and target (13°C) minimums
    targetTemperature = MIN_TEMPERATURE_C; // 13°C - meets target minimum
    isPowered = false;
    waterLevel = 100;
    isWaterLow = false;
    // Device properties
    ...
    lastScheduleActionTime = 0;
    failedUpdateAttempts = 0;
    updateInProgress = false;
    currentPollingInterval = POLLING_INTERVALS.BASE;
    // Debounced handlers
     ...
     */
    clearPendingTemperatureCommand() {
    ...
    updateSwitchServices() {
        ...
        if (this.temperatureSensorService) {
            ...
            this.temperatureSensorService.updateCharacteristic(this.platform.Characteristic.CurrentTemperature, validCurrentTemp);
        }
    }
    /**
     ...
     */
    updatePollingInterval(timeSinceUserAction, timeSinceScheduleAction) {
