...
export interface UltraConservativeRateLimiterConfig {
  // Based on empirical testing: 3-4 token bucket
  bucketCapacity: number;
  // Conservative refill rate: 1 token per 30 seconds
  refillIntervalMs: number;
  // Maximum burst size observed: 10 requests
  maxBurstSize: number;
  // Critical request bypass allowance
  allowCriticalBypass: boolean;
  // Emergency backoff on consecutive failures
...
export class UltraConservativeRateLimiter {
  private config: UltraConservativeRateLimiterConfig;
  ...
  constructor(config: Partial<UltraConservativeRateLimiterConfig> = {}) {
    this.config = {
      bucketCapacity: 2, // Ultra-conservative: 2 tokens (observed 3-4, using 50% margin)
      refillIntervalMs: 45000, // 45 seconds (observed 5-30s, using worst case + margin)
      maxBurstSize: 2, // Maximum 2 requests in burst
      allowCriticalBypass: true,
      emergencyBackoffMs: 120000, // 2 minutes emergency backoff
      startupGracePeriodMs: 180000, // 3 minutes startup grace
      ...config
    };

    this.bucket = {
      tokens: this.config.bucketCapacity, // Start with full bucket
   ...
   */
  public shouldAllowRequest(priority: RequestPriority): RequestDecision {
    const now = Date.now();
   ...
   */
  public recordRequest(priority: RequestPriority, success: boolean, rateLimited = false): void {
    const now = Date.now();
   ...
   */
  public getOptimalWaitTime(priority: RequestPriority): number {
